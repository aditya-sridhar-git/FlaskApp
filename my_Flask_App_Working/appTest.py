...
def submit_data():
    try:
        ...
        print(starting_Destination,ending_Destination)
...
def save_opted_path():
    ...
    if total_distance:
        ...
        with open(CSV_FILE, mode='a', newline='') as file:
            ...
            writer.writerow([f"{vehicleType}",f"{fuelType}",f"{total_distance}",f"{start_Lat}",f"{start_Lon}",f"{end_Lat}",f"{end_Lon}"])
...
def save_location_data():
    data = request.json
    starting_location = data.get('starting')
